...
<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST") {
        ...
        if(empty($_POST["depertman"])) {
            $depertmanErr = "depertman gerekli alan.";
        } else {
        ...
        if(empty($usernameErr) && empty($emailErr) && empty($passwordErr) && empty($repasswordErr)  && empty($telErr)) {
            $sql = "INSERT INTO kullanicilar(username,email,password  , tel ,depertman) VALUES(?,?,?,?,?)";

            if($stmt = mysqli_prepare($baglanti, $sql)) {
                $param_username = $username;
                $param_email = $email;
                $param_depertman = $depertman;
                $param_tel = $tel;
                $param_password = password_hash($password, PASSWORD_DEFAULT);

                mysqli_stmt_bind_param($stmt, "sssss", $param_username, $param_email, $param_password ,$tel, $depertman);
        ...
        <div class="col-12">
           <form action="register.php" method="post" novalidate>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="depertman" class="form-control" value="<?php echo $depertman;?>">
